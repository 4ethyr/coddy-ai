...
| `crates/coddy-voice-input` | Repositório Coddy ou crate compartilhada | Captura/transcrição de voz local e overlay com `VoiceInputConfig` neutro. |
...
- `coddy-voice-input` usa `VoiceInputConfig` neutro, sem depender de configuração VisionClip.
...
Veja tambem o [Manifesto de Separacao de Repositorios](repository-split-manifest.md) para a lista operacional de pacotes que devem ir com o Coddy e pacotes que devem permanecer no VisionClip.
...
O teste `crates/coddy-ipc/tests/repository_boundaries.rs` verifica que manifests e fontes Rust do Coddy não adicionem dependências diretas em crates runtime do VisionClip nem façam `include!` de arquivos de `apps/visionclip`.
...
O package `coddy-voice-input` já vive em `crates/coddy-voice-input`, então Coddy pode levar voz e overlay junto na separação física dos repositórios. Qualquer nova exceção deve ser explícita, documentada e removível antes da separação física.
