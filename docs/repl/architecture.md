...
coddy repl --terminal         # executa REPL stdin/stdout local com historico persistente
...
categoria, schemas de entrada/saida, risco, permissoes, timeout e approval
policy. O runtime ja devolve o catalogo rico para o CLI e para a UI Electron,
mantendo `ReplTools` apenas como compatibilidade com clientes legados.
...
contexto de system prompt e como evento observavel pela UI, sem declarar que um
subagent executou trabalho. O proximo passo arquitetural e conectar esse
registry a um executor com contexto isolado, eventos, auditoria e politicas
herdadas do tool registry.
